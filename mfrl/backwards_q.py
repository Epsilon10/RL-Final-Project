...
def learn(env, test_env, dims, num_episodes, gamma, alpha, eps, extract_state):
    ...
    for ep in range(num_episodes):
        ...
        for state, act, rew in zip(states, acts, rewards):
            next_q = 0. if step_num == len(states) - 1 else np.max(q_func[states[step_num + 1]])
            td = rew + gamma * next_q - q_func[state][act]
            q_func[state][act] += alpha * td
